...
public class BodyCondition {
    ...
    private BodyCondition(ResourceLocation name) {
        this.name = name;
        this.texture = ResourceHelper.ResourceLocation(name.getNamespace(), pathRoot + name.getPath());
    }

    public boolean abnormal(float value) {
        return defaultValue < value - Utils.EPS || defaultValue > value + Utils.EPS;
    }
    ...
    record ConditionBuilder(BodyCondition instance) {
        ...
        public ConditionBuilder isBone(Supplier<SurgeryBones> bone) {
            this.isSurgery();
