...
public class ConditionState implements Serializable {
    ...
    protected ConditionState(float value) {
    ...
    public boolean isDefault(BodyCondition condition) {
        if (Math.abs(this.value - condition.defaultValue()) > Utils.EPS) return false;
        if (Math.abs(this.hiddenValue) > Utils.EPS) return false;
        return this.tickCounter >= 20;
    }

    @Override
    public @NotNull CompoundTag serialize(HolderLookup.Provider provider) {
    ...
    @Override
    public void deserialize(HolderLookup.Provider provider, @NotNull CompoundTag tag) {
        this.build(
