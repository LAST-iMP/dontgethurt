...
public class InjuryEventHandler {
    ...
    private static float applyEquipment(LivingEntity entity, DamageSource source, InjuryDataSet injuryData, float damage) {
        ...
        if (!PlatformService.CONFIG.ARMOR_RECALCULATE()) {
            hurtArmor(damage, entity, source, EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.FEET, EquipmentSlot.LEGS);
            return CombatRules.getDamageAfterAbsorb(damage, valindaArmor, valindaRoughness);
        } else {
            ...
            for (int i = 0; i < size; i++) {
                ...
                hurtArmor(damage * 4 * data.factor() / slots.length, entity, source, slots);
                ...
                bodyDamage[i] = CombatRules.getDamageAfterAbsorb(damage, valindaArmor / size + bodyArmor, valindaRoughness / size + bodyRoughness);
    ...
    private static void hurtArmor(float damage, LivingEntity livingEntity, DamageSource source, EquipmentSlot ...slots) {
        if (damage <= 0.0F) return;
        var armors = Arrays.stream(slots).mapToInt(EquipmentSlot::getIndex).toArray();

        if (livingEntity instanceof Player player) {
            player.getInventory().hurtArmor(source, damage, armors);
        } else {
            var armorDamage = Math.max(1f, damage / 4);
            Arrays.stream(slots).forEach(slot -> {
                ...
                if ((!source.is(DamageTypeTags.IS_FIRE) || !itemstack.getItem().isFireResistant()) && itemstack.getItem() instanceof ArmorItem) {
                    itemstack.hurtAndBreak((int)armorDamage, livingEntity, entity ->
                            entity.broadcastBreakEvent(EquipmentSlot.byTypeAndIndex(EquipmentSlot.Type.ARMOR, slot.getIndex()))
                    );
                }
            });
        }
