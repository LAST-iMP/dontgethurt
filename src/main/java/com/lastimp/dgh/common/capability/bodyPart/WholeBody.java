...
public class WholeBody implements Serializable {
    ...
    public CompoundTag serialize(HolderLookup.Provider provider) {
        CompoundTag tag = new CompoundTag();
        this.components.keySet().forEach(key -> tag.put(key.name(), this.getComponent(key).serialize(provider)));
        return tag;
    }
    ...
    public void deserialize(HolderLookup.Provider provider, CompoundTag nbt) {
        this.components.keySet().forEach(key -> components.get(key).deserialize(provider, nbt.getCompound(key.name())));
    }
