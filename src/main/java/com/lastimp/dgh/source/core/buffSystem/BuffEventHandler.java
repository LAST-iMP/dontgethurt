...
public class BuffEventHandler {
    ...
    private static void updateStaggerEffects(HealthCapability health, LivingEntity entity) {
        if (health.slowDown() > 0 && !entity.hasEffect(ModEffects.STAGGER_EFFECT.get())) {
    ...
    private static void updateWithdrawEffects(HealthCapability health, LivingEntity entity) {
        ...
        if (state.getValue() > 0.2f && !entity.hasEffect(ModEffects.CRAVING_EFFECT.get())) {
            entity.addEffect(new MobEffectInstance(ModEffects.CRAVING_EFFECT.get(), 100));
        }
        if (state.getValue() > 0.3f && !entity.hasEffect(ModEffects.SWEATING_EFFECT.get())) {
            entity.addEffect(new MobEffectInstance(ModEffects.SWEATING_EFFECT.get(), 100));
        }
    ...
    private static void updateLivingTimeEffects(HealthCapability health, LivingEntity entity) {
        if (entity.hasEffect(ModEffects.KEEP_LIVING_EFFECT.get())) return;
    ...
    private static void updateCureEffects(HealthCapability health, LivingEntity entity) {
        if (entity.hasEffect(ModEffects.CURE_EFFECT.get())) return;
        if (health.vitality() < 0.999f) return;
        if (health.almostDead() < 0.2f) {
            entity.addEffect(new MobEffectInstance(ModEffects.CURE_EFFECT.get(), 2400, 2));
        } else if (health.almostDead() < 0.5f) {
            entity.addEffect(new MobEffectInstance(ModEffects.CURE_EFFECT.get(), 2400, 1));
        } else if (health.almostDead() < 0.8f) {
            entity.addEffect(new MobEffectInstance(ModEffects.CURE_EFFECT.get(), 2400, 0));
        } else {
            return;
        }
    ...
    private static void updateSymptomsEffects(HealthCapability health, LivingEntity entity) {
        if (!entity.hasEffect(ModEffects.INTENSE_PAIN_EFFECT.get()) && health.intensePain()) {
        ...
        if (!entity.hasEffect(ModEffects.PALE_SKIN.get()) && health.getComponent(BLOOD).getConditionValue(BLOOD_LOSS) > 0.4f) {
            entity.addEffect(new MobEffectInstance(ModEffects.PALE_SKIN.get(), 100));
        }
        if (!entity.hasEffect(ModEffects.HARD_BREATH.get())) {
            if (health.getComponent(BLOOD).getConditionValue(OXYGEN) > 0.2f || health.getComponent(TORSO).abnormal(RESPIRATORY_ARREST))
                entity.addEffect(new MobEffectInstance(ModEffects.HARD_BREATH.get(), 100));
        }
